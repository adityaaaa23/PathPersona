...
const Dashboard = () => {
  ...
  if (!user)
    return (
      <div className="min-h-screen flex items-center justify-center px-6 bg-brand-cream text-brand-charcoal">
        <div className="border-2 border-brand-charcoal bg-brand-white p-8 text-center swiss-shadow">
          <p className="text-sm font-black uppercase tracking-wider mb-4">
            {error || "Dashboard unavailable."}
          </p>
          <button
            onClick={() => window.location.reload()}
            className="bg-brand-red text-brand-white border-2 border-brand-charcoal px-6 py-3 text-xs font-black tracking-widest uppercase cursor-pointer"
          >
            Try Again
          </button>
        </div>
    ...
    <div className="min-h-screen w-full max-w-full flex flex-col px-4 py-6 gap-6 overflow-x-hidden md:flex-row md:items-stretch md:gap-8 font-sans pt-28 pb-16 bg-brand-cream">
      ...
      <aside className="hidden md:flex md:flex-col md:items-center md:justify-between md:w-1/4 bg-brand-white border-2 border-brand-charcoal p-6 shadow-xl swiss-shadow-red rounded-none">
        ...
        <div className="w-full text-center border-t-2 border-brand-red pt-4">
          <p className="text-[10px] font-black tracking-widest uppercase text-brand-charcoal/70 font-mono">
            {user.email}
          </p>
        </div>
      </aside>
      ...
      <main className="flex flex-col gap-6 w-full max-w-full overflow-x-hidden p-6 md:w-3/4 md:p-8 bg-brand-white border-2 border-brand-charcoal rounded-none shadow-2xl swiss-shadow">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b-2 border-brand-red pb-5">
          <div className="flex flex-wrap items-baseline gap-2">
            <p className="text-brand-charcoal/70 text-xs font-bold uppercase tracking-wider">
              {isNewUser ? "Hello," : "Good to see you back,"}
            </p>
            <h1 className="capitalize text-xl font-black tracking-wide text-brand-charcoal uppercase">
              {user.username}
            </h1>
          </div>
          <div>
        ...
        {isNewUser && (
          <div className="border-2 border-dashed border-brand-red bg-brand-red/5 p-6 rounded-none text-left flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <p className="font-black text-brand-charcoal uppercase tracking-wider mb-1">
                Welcome to your learning journey!
              </p>
              <p className="text-xs text-brand-charcoal/80 leading-relaxed font-bold uppercase tracking-wide">
                Take the learning style quiz first so we can automatically shape
                all generated roadmaps to your personal learning speed and
                formats.
              </p>
            </div>
            <button
              className="flex-shrink-0 text-xs font-black tracking-widest uppercase text-brand-white bg-brand-red border-2 border-brand-charcoal px-6 py-3 hover:bg-brand-charcoal hover:text-brand-cream hover:border-brand-charcoal transition-all duration-200 cursor-pointer shadow-[2px_2px_0px_0px_rgba(21,21,21,1)]"
              onClick={() => navigate("/quiz")}
            >
              Take Quiz →
            </button>
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          <StatCard
            ...
            value={stats?.numberOfRoadmapsGenerated ?? 0}
          />
          <StatCard
            label="Completed"
            value={stats?.numberOfRoadmapsFinished ?? 0}
          />
          <StatCard
        ...
        {quote && (
          <div className="rounded-none px-6 py-4 border-2 border-brand-red bg-brand-cream text-center relative overflow-hidden">
            <p className="text-xs font-black uppercase tracking-wider text-brand-charcoal/85 leading-relaxed relative z-10">
              "{quote}"
            </p>
          </div>
        )}
        ...
        <div className="flex justify-between items-center mt-2">
          ...
          <button
            className="text-xs font-black tracking-widest uppercase px-4 py-2 bg-brand-cream text-brand-charcoal hover:bg-brand-charcoal hover:text-brand-white border-2 border-brand-charcoal rounded-none transition-all duration-200 cursor-pointer"
            onClick={newRoadmap}
          >
            + New Roadmap
          </button>
        ...
        {roadmaps.length === 0 ? (
          <div className="py-8 text-center border-2 border-dashed border-brand-red rounded-none bg-brand-cream">
            <p className="text-xs font-black uppercase tracking-wider text-brand-charcoal/70">
              {isNewUser
                ? "Complete the personality quiz first, then generate your first study roadmap!"
                : "No roadmaps created yet. Generate one now to start learning."}
            </p>
          </div>
        ) : (
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-4 md:flex-row md:flex-wrap">
              {roadmaps.slice(0, 4).map((r) => {
                const completedDays =
                  r.roadmap?.filter((d) => d.completed).length ?? 0;
                const totalDays = r.roadmap?.length ?? 28;
                const progress = Math.round((completedDays / totalDays) * 100);
                const lastAttempt =
                  r.quizHistory?.length > 0
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start px-4 py-3.5 border-b-2 border-brand-charcoal bg-brand-cream/60">
                      <div>
                        <p className="text-sm font-black uppercase tracking-wide text-brand-charcoal mb-1 truncate max-w-[125px]">
                          {r.skill}
                        </p>
                        <span className="text-[9px] font-black uppercase tracking-widest text-brand-white bg-brand-red border border-brand-charcoal px-2.5 py-0.5 rounded-none">
                          {r.personalityType}
                        </span>
                      </div>
                      {r.completed && (
                    ...
                    <div className="flex flex-col gap-2 px-4 py-3.5 border-b-2 border-brand-charcoal bg-brand-cream/20">
                      <div className="flex justify-between items-center">
                        <span className="text-[9px] font-black uppercase tracking-wider text-brand-charcoal/60">
                          Progress
                        </span>
                        <span className="text-[10px] text-brand-charcoal font-mono font-bold">
                    ...
                    {lastAttempt !== null && (
                      <div className="flex justify-between items-center px-4 py-2 border-b-2 border-brand-charcoal bg-brand-cream/20">
                        ...
                        <span
                          className={`text-xs font-black font-mono ${lastAttempt >= 75 ? "text-emerald-600" : "text-brand-red"}`}
                        >
                          {lastAttempt}%
                        </span>
                      </div>
                    ...
                    <div className="flex justify-between items-center px-4 py-3 bg-brand-cream/40">
                      <span className="text-[9px] font-black uppercase tracking-wider text-brand-charcoal/50 font-mono">
                        {r.quizHistory?.length ?? 0} attempt
                        {r.quizHistory?.length !== 1 ? "s" : ""}
                      </span>
                      <span className="text-brand-red text-xs font-black uppercase tracking-widest hover:underline">
                        View →
                      </span>
                    </div>
                  </div>
            ...
            {roadmaps.length > 4 && (
              <button
                className="mt-2 text-xs font-black tracking-widest uppercase text-brand-red hover:underline transition-all duration-200 self-center md:self-start"
                onClick={() => navigate("/roadmaps")}
              >
